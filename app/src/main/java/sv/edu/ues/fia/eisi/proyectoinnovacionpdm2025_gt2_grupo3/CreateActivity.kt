...
class CreateActivity : AppCompatActivity() {
    ...
    private lateinit var editTopText: EditText
    private lateinit var editBottomText: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editTopText = findViewById(R.id.editTopText)
        editBottomText = findViewById(R.id.editBottomText)
    ...
    private fun updateMemePreview() {
        if (selectedImageUri != null) {
            try {
                ...
                // Obtener los textos
                val topText = editTopText.text.toString()
                val bottomText = editBottomText.text.toString()
                ...
                val textWidthTop = paint.measureText(topText)
                val textWidthBottom = paint.measureText(bottomText)
                val xPosTop = (mutableBitmap.width - textWidthTop) / 2
                val xPosBottom = (mutableBitmap.width - textWidthBottom) / 2

                // Dibujar texto en el bitmap
                if (topText.isNotEmpty()) {
                    canvas.drawText(topText, xPosTop, 100f, paint)
                }
                if (bottomText.isNotEmpty()) {
                    canvas.drawText(bottomText, xPosBottom, mutableBitmap.height - 50f, paint)
                }

                // Mostrar el bitmap resultante
